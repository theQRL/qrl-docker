# QRL node on Ubuntu 18.04 (Bionic).
#
...
#
# 18.04 is past standard support and ships Python 3.6. It can only build
# theQRL/QRL master, whose pins (Twisted 20.3.0, cryptography 2.3, click 8.0)
# still admit 3.6 — noble-work-in-progress cannot be installed here at all.
#
# Branch-specific values (base tag, Python 3.6, runtime library package names)
...
# NB: no PIP_NO_CACHE_DIR here. The pip 9.0.1 that python3.6's venv bundles
# reads it as a value rather than a flag, so "=1" crashes every pip invocation
# with "TypeError: expected str, bytes or os.PathLike object, not int" — before
# the pip upgrade below has a chance to run. The cache is removed explicitly at
...
RUN set -eux; \
    ...
    && pip install --upgrade "pip<22" "setuptools<60" wheel \
...
LABEL org.opencontainers.image.title="qrl-docker" \
      org.opencontainers.image.description="QRL node on Ubuntu 18.04 Bionic" \
