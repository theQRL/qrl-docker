# QRL node on Ubuntu 22.04 (Jammy).
#
...
#
# Branch-specific values (base tag, Python 3.10, runtime library package names)
...
# theQRL/QRL noble-work-in-progress: the QRL source is upstream theQRL, but
# its requirements.txt pulls pyqrllib / pyqryptonight / pyqrandomx from the
# jplomas forks, which carry the modern-Linux rework. That rework has NOT been
# exercised on a production testnet, so images from this branch are TEST
# builds. See README.md.
#
# Upstream theQRL/QRL master does NOT build here, and it is not close. master
# pins the 2018-era set — cryptography==2.3, Twisted==20.3.0, pyqrllib 1.2.4 —
# which 22.04 cannot satisfy:
#   * those sdists' setup.py files import pkg_resources, which setuptools >= 81
#     no longer ships. PEP 517 isolation installs the newest setuptools into a
#     throwaway env and PIP_CONSTRAINT is NOT applied there (verified on pip
#     26.2.1), so the pin cannot be forced.
#   * building without isolation fixes that but breaks Twisted 20.3.0, whose
#     incremental-based versioning then resolves to 0.0.0 and pip discards it.
#   * cryptography==2.3 predates OpenSSL 3.0, which is what 22.04 ships.
# 20.04 is the newest release that can still build upstream master.
ARG QRL_REF=noble-work-in-progress
...
LABEL org.opencontainers.image.title="qrl-docker" \
      org.opencontainers.image.description="QRL node on Ubuntu 22.04 Jammy" \
      ...
      org.theqrl.qrl.ref="noble-work-in-progress" \
      org.theqrl.deps="test"
